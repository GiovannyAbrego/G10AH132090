    ...
    if(!!this.state.usuario && !!this.state.contrasena){
      ...
      .then((responseJson)=>{
        ...
        if(encontrado=='si'){
          ...
          Alert.alert('Usuario','No encontrado!!',[{text:'OK',onPress:()=>console.log('Ok Pressed')},],{cancelable:false})
        }
      })
      .catch((error)=>{console.error(error);
        Alert.alert('Aviso', 'Error de Internet!!', [{ text: 'OK', onPress: () => console.log('Ok Pressed') },], { cancelable: false })
      })
    }else{
      Alert.alert('Aviso', 'No introdujo datos', [{ text: 'OK', onPress: () => console.log('Ok Pressed') },], { cancelable: false })
    }
  }
  render(){
    return(
      <View style={{flex:1,padding:10}}>
        ...
        <Image style={{ width: 200, height: 160, alignSelf: 'center', marginTop: 15 }} source={require('./src/img/market.jpg')}/>
        <View style={{marginLeft:10,marginRight:10}}>
          <Input placeholder='USUARIO' onChangeText={(text)=>this.setState({usuario:text})}
          rightIcon={<Icon name='user' size={24} color='black'/>}/>
          <Input placeholder='CONTRASEÑA' onChangeText={(text) => this.setState({ contrasena: text })}
            rightIcon={<Icon name='lock' size={24} color='black' />} />
        </View>
        <TouchableOpacity style={{height:50,backgroundColor:'red',marginTop:15,borderRadius:5,justifyContent:'center',marginLeft:20,marginRight:20}} onPress={()=>{this.Entrar()}}>
...
class listarProductos extends React.Component{
  ...
  render(){
    ...
    <View style={{flex:1}}>
      ...
      <FlatList data={this.state.elementos}
      renderItem={
        ({item})=>
          <TouchableOpacity key={item.id} onPress={()=>this.props.navigation.navigate('Detalles',item)}>
      ...
      <View style={{flex:1}}>
        <ScrollView style={{flex:1}}>
          <View style={{flex: 1, flexDirection: 'row', alignContent: 'center', alignItems: 'center', height: 60}}>
            <TouchableOpacity style={{flex: 1, height:40, backgroundColor: 'black', borderRadius: 5, justifyContent:'center', marginLeft: 5}}
            onPress={() => { this.Actualizar() }}>
              <Text style={{color: 'white', fontSize: 22, textAlign: 'center',textAlignVertical: 'center', padding: 3}}>
                  Actualizar
              </Text>
            </TouchableOpacity>
            <TouchableOpacity style={{ flex: 1, height: 40, backgroundColor: 'black', borderRadius: 5, justifyContent: 'center', marginLeft: 5,marginRight:5}}
